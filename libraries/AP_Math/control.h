...
// Determines how externally supplied trajectory acceleration is combined with
// acceleration generated by a controller to correct tracking error.
enum class AccelTargetAllocation : uint8_t {
    Legacy,
    CorrectionPriority,
};

// Combines a 2D corrective acceleration with a lower-priority trajectory
// acceleration. Correction may use the full envelope; trajectory acceleration
// is admitted only from the remaining magnitude. Correction is never limited
// here, leaving the final controller limiter responsible for saturation and
// anti-windup.
Vector2f allocate_accel_correction_priority(const Vector2f& correction, const Vector2f& trajectory, float accel_max);

// Single-axis form of allocate_accel_correction_priority().
