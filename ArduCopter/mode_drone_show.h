...
private:
    ...
    // the XY error gate is enabled, the landing stage first holds altitude
    // while moving to the intended landing position and starts the descent
    // only when the horizontal error drops below the limit (or a timeout
    // passes).
